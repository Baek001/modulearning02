...
$runtimeFixesSql = Join-Path $ddlPath "runtime_fixes_postgres.sql"
...
$demoSeedSql = Join-Path $ddlPath "seed_operational_demo.sql"
$communitySeedSql = Join-Path $ddlPath "seed_community_demo.sql"
$boardSeedSql = Join-Path $ddlPath "seed_board_demo.sql"
$calendarSeedSql = Join-Path $ddlPath "seed_calendar_demo.sql"
$contractSeedSql = Join-Path $ddlPath "seed_contract_demo.sql"
...
if (Test-Path -LiteralPath $runtimeFixesSql) {
    Invoke-SqlFileInContainer -Container $ContainerName -FilePath $runtimeFixesSql -User $PgUser -Database $PgDatabase
}
...
if (Test-Path -LiteralPath $demoSeedSql) {
    Invoke-SqlFileInContainer -Container $ContainerName -FilePath $demoSeedSql -User $PgUser -Database $PgDatabase
}
if (Test-Path -LiteralPath $communitySeedSql) {
    Invoke-SqlFileInContainer -Container $ContainerName -FilePath $communitySeedSql -User $PgUser -Database $PgDatabase
}
if (Test-Path -LiteralPath $boardSeedSql) {
    Invoke-SqlFileInContainer -Container $ContainerName -FilePath $boardSeedSql -User $PgUser -Database $PgDatabase
}
if (Test-Path -LiteralPath $calendarSeedSql) {
    Invoke-SqlFileInContainer -Container $ContainerName -FilePath $calendarSeedSql -User $PgUser -Database $PgDatabase
}
if (Test-Path -LiteralPath $contractSeedSql) {
    Invoke-SqlFileInContainer -Container $ContainerName -FilePath $contractSeedSql -User $PgUser -Database $PgDatabase
}
